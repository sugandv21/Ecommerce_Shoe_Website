...
const ContactPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModal({
        show: true,
        title: "Success",
        message: `Thank you for reaching us, ${formData.name}. We will contact you at ${formData.email}`,
        type: "success"
      });
  ...
  const renderDescription = () => {
    ...
    return desc.replace(/\r\n/g, '\n').split('\n').map((line, idx) => {
      if (line.startsWith('## '))
        return <h3 key={idx} className="text-xl font-semibold mt-6 mb-2">{line.slice(3)}</h3>;
      if (line.trim() === '')
        return <br key={idx} />;
